...
export function unzip<K, R extends number>(
  ...
  const [__left, __right] = UnrolledList.splitAt(root.items, index + 1);
  // Otherwise, we found the key (or a larger key)...
  // const [_key, item] = root.items.at(index)!;
  // const [_key, item] = __left.last()!;
  const [_key, item] = __left.pop()!;
  if (_key === key) {
    ...
    if (drop) {
      ...
      // Put them back on the left side to keep
      __left.push([_key, item]);
    }
